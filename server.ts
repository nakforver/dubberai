...
function cleanJsonResponse(raw: string): any {
  ...
  try {
    ...
    if (startIdx !== -1 && endIdx !== -1 && endIdx > startIdx) {
      const sub = text.slice(startIdx, endIdx + 1);
      return JSON.parse(sub);
    }
    throw new Error(`AI returned invalid JSON: ${text.slice(0, 120)}...`);
  }
}
...
async function callOpenAiCompatibleChat({
  ...
  const root = (baseUrl || 'https://codecraftapi.com/v1').replace(/\/+$/, '');
...
async function translateSubtitleLinesWithAI({
  sourceLines,
  aiProvider = 'gemini',
  model,
  apiKey,
  ...
  console.log(
    `[TRANSLATE] Translating ${sourceLines.length} subtitle lines using ${activeProvider} (${model || 'default'})`
  );
  ...
  for (let i = 0; i < sourceLines.length; i += CHUNK_SIZE) {
    ...
    while (!success && retries < 3) {
      try {
        ...
        if (activeProvider === 'codecraft' || activeProvider === 'custom') {
          const effectiveBaseUrl = baseUrl || (activeProvider === 'codecraft' ? 'https://codecraftapi.com/v1' : 'https://api.openai.com/v1');
          const effectiveModel = model || (activeProvider === 'codecraft' ? 'claude-sonnet-5' : 'gpt-4o');

          const prompt = `You are a professional subtitle translator.
Translate the following subtitle JSON array into natural, culturally fluent Khmer (Cambodian).

STRICT RULES:
1. Translate ONLY the "text" field into Khmer.
2. NEVER change "id", "start", or "end".
3. Return EXACTLY the same number of subtitle objects (${chunk.length} items).
4. Keep the EXACT same object order.
5. Do NOT merge or split subtitle lines.
6. Return ONLY a valid JSON array of objects. No markdown formatting, no explanations.

SUBTITLES TO TRANSLATE:
${JSON.stringify(chunk)}`;

          const aiReply = await callOpenAiCompatibleChat({
            ...
            messages: [
              {
                role: 'system',
                content: 'You are an expert subtitle translator specialized in Khmer (Cambodian). You output strictly valid JSON array only.',
              },
          ...
          parsed = cleanJsonResponse(aiReply);
        } else {
          // Gemini
          if (!currentAi) {
            throw new Error('Gemini API client not initialized.');
          }
          const activeGeminiModel = model && !['amazon'].includes(model) ? model : 'gemini-2.5-flash';
...
${JSON.stringify(chunk)}`
        ...
        if (parsed.length !== chunk.length) {
          throw new Error(
            `AI returned ${parsed.length} lines, expected ${chunk.length}.`
          );
        }
        ...
        if (onProgress) {
          const percent = Math.floor(Math.min(100, ((i + CHUNK_SIZE) / sourceLines.length) * 100));
          onProgress(percent);
        }
      } catch (err: any) {
        lastError = err;
        console.error(`[TRANSLATE] Chunk ${i} error:`, err?.message || err);
        retries++;
        if (retries < 3) {
    ...
    if (!success) {
      throw new Error(
        `Translation via ${activeProvider} failed after 3 attempts: ${lastError?.message || lastError}`
      );
    }
  }
...
async function translateSubtitleLinesToKhmer(
  ...
  return translateSubtitleLinesWithAI({
    sourceLines,
    aiProvider: 'gemini',
    model: geminiModel,
    currentAi,
...
app.post('/api/transcribe/start', async (req, res) => {
  ...
  const headerCustomApiKey = (req.headers['x-custom-api-key'] as string) || '';
  const effectiveCustomApiKey = customApiKey || headerCustomApiKey;
  ...
  (async () => {
    try {
      ...
      const activeProvider = aiProvider || (model?.startsWith('claude-') || model?.startsWith('deepseek-') || model?.startsWith('gpt-5') || model?.startsWith('qwen3') ? 'codecraft' : 'gemini');

      if (activeProvider === 'gemini' && !currentAi) {
        throw new Error(
          'Gemini API Key មិនត្រូវបានកំណត់។ សូមបញ្ចូល Gemini API Key ក្នុង App Settings។'
        );
      }
      if ((activeProvider === 'codecraft' || activeProvider === 'custom') && !effectiveCustomApiKey && !apiKey) {
        throw new Error(
          `API Key សម្រាប់ ${activeProvider === 'codecraft' ? 'CodeCraft' : 'Custom Provider'} មិនទាន់បានកំណត់។ សូមបញ្ចូល API Key ក្នុង App Settings។`
        );
      }
...
if (activeModel === 'amazon') {
         ...
         console.log(`Translating ${originalLines.length} lines to Khmer using ${activeProvider}...`);
         
         const translatedLines = await translateSubtitleLinesWithAI({
           sourceLines: originalLines,
           aiProvider: activeProvider,
           model: activeProvider !== 'gemini' ? (customModel || model) : activeGeminiModel,
           apiKey: activeProvider !== 'gemini' ? effectiveCustomApiKey : apiKey,
           baseUrl: customBaseUrl,
           currentAi,
           onProgress: (percent) => {
             const translateProgress = 90 + Math.floor((percent / 100) * 9);
...
app.post('/api/test-ai-provider', async (req, res) => {
  try {
    const { provider = 'gemini', apiKey, baseUrl, model } = req.body;

    if (provider === 'codecraft' || provider === 'custom') {
      const effectiveBaseUrl = baseUrl || (provider === 'codecraft' ? 'https://codecraftapi.com/v1' : 'https://api.openai.com/v1');
      const effectiveModel = model || (provider === 'codecraft' ? 'claude-sonnet-5' : 'gpt-4o');

      if (!apiKey) {
        return res.status(400).json({ 
          error: `សូមបញ្ចូល API Key សម្រាប់ ${provider === 'codecraft' ? 'CodeCraft API' : 'Custom Provider'} ជាមុនសិន។` 
        });
      }
      ...
      const reply = await callOpenAiCompatibleChat({
        ...
        model: effectiveModel,
        messages: [
          { role: 'system', content: 'You are a helpful AI assistant. Respond with a short polite confirmation in Khmer.' },
          { role: 'user', content: 'Say: "សួស្តីពី DubberAI! ការតភ្ជាប់ទៅកាន់ AI ទទួលបានជោគជ័យ។"' }
        ],
        temperature: 0.3,
      });
      ...
      return res.json({
        success: true,
        message: `ជោគជ័យ! ការតភ្ជាប់ទៅកាន់ ${effectiveModel} (${provider === 'codecraft' ? 'CodeCraft' : 'Custom'}) ដំណើរការល្អ។`,
        reply: reply.trim(),
      });
    } else {
      // Gemini
      ...
      if (!key) {
        return res.status(400).json({ 
          error: 'សូមបញ្ចូល Gemini API Key ជាមុនសិន។' 
        });
      }
      ...
      const response = await client.models.generateContent({
        ...
        contents: [{
          role: 'user',
          parts: [{ text: 'Respond with: "សួស្តីពី DubberAI! ការតភ្ជាប់ទៅកាន់ Google Gemini ទទួលបានជោគជ័យ។"' }]
        }]
      });

      return res.json({
        success: true,
        message: `ជោគជ័យ! ការតភ្ជាប់ទៅកាន់ ${activeModel} (Google Gemini) ដំណើរការល្អ។`,
    ...
    return res.status(400).json({
      error: `ការតភ្ជាប់បរាជ័យ: ${error.message || error}`
    });
  }
});
...
app.post('/api/ai-agent/chat', async (req, res) => {
  try {
    ...
    const subtitlesContext = subtitles && subtitles.length > 0
      ? `\nCURRENT VIDEO SUBTITLES (${subtitles.length} lines):\n` +
        JSON.stringify(subtitles.slice(0, 50).map((s: any) => ({ id: s.id, text: s.text, start: s.start, end: s.end })), null, 2)
      : '\nNo subtitles currently loaded in editor.';

    const systemPrompt = `You are DubberAI Agent, a helpful, intelligent AI assistant and video dubbing expert.
You help users with video dubbing, subtitle translation, and Khmer language localization.
Key guidelines:
1. Always respond politely, naturally, and fluently in Khmer (or match the language used by the user).
2. If asked to summarize the video, read the subtitles context below and provide a concise, engaging summary.
3. If asked to polish, improve, or proofread subtitles:
   - Provide the natural phrasing in Khmer.
   - Keep sentences natural for Edge TTS voice narration.
   - If recommending specific subtitle changes, include a clean JSON block in this format:
     \`\`\`json
     [
       { "id": "1", "text": "អត្ថបទកែលម្អ..." }
     ]
     \`\`\`
     This allows the user to apply updates to their timeline with one click.
4. If asked general questions, answer helpfully and concisely.
${subtitlesContext}`;
    ...
    if (provider === 'codecraft' || provider === 'custom') {
      ...
      if (!effectiveApiKey) {
        return res.status(400).json({ error: 'សូមកំណត់ API Key សម្រាប់ AI Provider ជាមុនសិន' });
      }
      ...
      return res.json({ reply: reply.trim(), model: effectiveModel, provider });
    } else {
      // Gemini
      const key = apiKey ? apiKey.trim() : (process.env.GEMINI_API_KEY || '');
      const client = key ? new GoogleGenAI({ apiKey: key }) : ai;
      if (!client) {
        return res.status(400).json({ error: 'Gemini API Key មិនទាន់បានកំណត់' });
      }
      const activeModel = model || 'gemini-2.5-flash';
      ...
      return res.json({ reply: (response.text || '').trim(), model: activeModel, provider: 'gemini' });
    }
  } catch (error: any) {
    console.error('AI Agent chat error:', error);
    return res.status(500).json({ error: error.message || 'AI Agent error' });
  }
});
