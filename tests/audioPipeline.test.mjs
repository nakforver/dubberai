...
test('converts SRT to ASS with correct PlayRes, Noto Sans Khmer font, and intact Khmer typography', () => {
  ...
  assert.match(assPortrait, /MarginV, Encoding\nStyle: Default,Noto Sans Khmer,44/);
});

test('cleanJsonResponse extracts and parses JSON even with markdown wraps or conversational commentary', () => {
  const plain = '[{"id":"1","text":"សួស្តី"}]';
  assert.deepEqual(buildAndValidateSegments.cleanJsonResponse(plain), [{ id: '1', text: 'សួស្តី' }]);

  const markdown = '```json\n[{"id":"2","text":"អរគុណ"}]\n```';
  assert.deepEqual(buildAndValidateSegments.cleanJsonResponse(markdown), [{ id: '2', text: 'អរគុណ' }]);

  const withCommentary = 'Here is your translated subtitles in Khmer:\n[{"id":"3","text":"សូមស្វាគមន៍"}]\nEnjoy your dubbing!';
  assert.deepEqual(buildAndValidateSegments.cleanJsonResponse(withCommentary), [{ id: '3', text: 'សូមស្វាគមន៍' }]);
});

test('translateSubtitleLinesWithAI routes through CodeCraft/OpenAI-compatible endpoints', async () => {
        ...
        choices: [
          {
            message: {
              content: JSON.stringify([
                { id: 'sub-1', start: '00:01.0', end: '00:04.0', text: 'សួស្តីពិភពលោក' },
                { id: 'sub-2', start: '00:04.5', end: '00:07.0', text: 'រីករាយដែលបានជួបអ្នក' }
              ])
            }
          }
        ]
  ...
  try {
    const source = [
      { id: 'sub-1', start: '00:01.0', end: '00:04.0', text: 'Hello world' },
      { id: 'sub-2', start: '00:04.5', end: '00:07.0', text: 'Nice to meet you' }
    ];

    const result = await buildAndValidateSegments.translateSubtitleLinesWithAI({
      sourceLines: source,
      aiProvider: 'codecraft',
      model: 'claude-sonnet-5',
      apiKey: 'test-codecraft-key',
      baseUrl: 'https://codecraftapi.com/v1'
    });

    assert.equal(requestedUrl, 'https://codecraftapi.com/v1/chat/completions');
    assert.equal(requestedAuth, 'Bearer test-codecraft-key');
    assert.equal(requestedBody.model, 'claude-sonnet-5');
    assert.equal(result.length, 2);
    assert.equal(result[0].id, 'sub-1');
    assert.equal(result[0].start, '00:01.0');
    assert.equal(result[0].end, '00:04.0');
    assert.equal(result[0].text, 'សួស្តីពិភពលោក');
    assert.equal(result[1].text, 'រីករាយដែលបានជួបអ្នក');
  } finally {
